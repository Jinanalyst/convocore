...
import { Mic, MicOff, Volume2, VolumeX, Sparkles, Loader2, AlertCircle } from "lucide-react";
...
export function VoiceChat({
  ...
  useEffect(() => {
    const checkSupport = () => {
      try {
        ...
        setIsSupported(false);
        setError('Speech services not available');
      }
    };
  ...
  // Demo mode simulation (if enabled)
  useEffect(() => {
    if (!demoMode) return;
    ...
    <div className={cn("flex flex-col items-center justify-center min-h-screen bg-background relative overflow-hidden", className)}>
      ...
      <div className="relative z-10 flex flex-col items-center space-y-8">
        ...
        <div className="text-center space-y-2">
          ...
          {transcript && (
            <motion.div
              className="max-w-md mx-auto p-3 bg-muted/20 rounded-lg border"
