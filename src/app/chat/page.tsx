...
const SidebarDynamic = dynamic(() => import("@/components/layout/sidebar").then(mod => ({ default: mod.Sidebar })), {
  ssr: false,
  loading: () => (
    <div className="w-80 bg-white dark:bg-zinc-900 border-r border-gray-200 dark:border-zinc-800">
      <div className="p-4">
...
const HeaderDynamic = dynamic(() => import("@/components/layout/header").then(mod => ({ default: mod.Header })), {
  ssr: false,
  loading: () => (
    <div className="h-16 bg-white dark:bg-zinc-800 border-b border-gray-200 dark:border-zinc-700">
...
function ChatPageContent() {
  ...
  const handleSelectChat = async (threadId: string) => {
    router.replace(`/chat?chatId=${threadId}`);
  ...
  const handleSendMessage = async (message: string, model: string, includeWebSearch?: boolean) => {
    ...
    try {
      ...
      // Store messages on Solana if wallet is connected
      const walletConnected = localStorage.getItem('wallet_connected') === 'true';
      if (walletConnected && currentThreadId) {
        try {
          // Store user message
          const userMsgResponse = await fetch('/api/wallet/messages', {
            method: 'POST',
          ...
          if (userMsgResponse.ok) {
            ...
            if (userMsgData.transaction) {
              // Request wallet signature
              const signedTx = await requestWalletSignature(userMsgData.transaction);
              if (signedTx) {
          ...
          const asstMsgResponse = await fetch('/api/wallet/messages', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              conversationId: currentThreadId,
          ...
          if (asstMsgResponse.ok) {
            const asstMsgData = await asstMsgResponse.json();
            if (asstMsgData.transaction) {
              // Request wallet signature
              const signedTx = await requestWalletSignature(asstMsgData.transaction);
              if (signedTx) {
                await submitTransaction(signedTx);
              }
            }
          ...
          console.error('Failed to store messages on Solana:', error);
        }
      }
  ...
  // Helper function to request wallet signature
  const requestWalletSignature = async (serializedTransaction: string): Promise<string | null> => {
    try {
      // This would integrate with the user's wallet (Phantom, Solflare, etc.)
      // For now, we'll return null to indicate no signature
  ...
  const submitTransaction = async (signedTransaction: string): Promise<void> => {
    try {
    ...
    if (walletConnected) {
      try {
        ...
        if (response.ok) {
          ...
          // If there's a transaction to sign, request signature
          if (data.transaction) {
            const signedTx = await requestWalletSignature(data.transaction);
            if (signedTx) {
              await submitTransaction(signedTx);
            }
          }
