  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!isSupabaseConfigured) {
        setError('Google authentication is not configured. Please use wallet login or contact support.');
        return;
      }
      ...
      if (error) {
        ...
        if (error.message.includes('validation_failed') || error.message.includes('provider is not enabled')) {
          setError('Google authentication is not enabled. Please contact support or use wallet login.');
            ...
            {!isSupabaseConfigured && (
              <div className="bg-blue-900/20 border border-blue-500 text-blue-400 px-4 py-3 rounded">
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <h3 className="text-sm font-medium text-blue-400">Google Login Unavailable</h3>
                    <p className="mt-1 text-sm text-blue-300">
                      Google authentication is not configured. Please use wallet login or contact support for setup instructions.
                    </p>
                  </div>
                </div>
              </div>
